...
class SecurityController extends Controller
{
   ...
   *
   */
    public function passwordResetFormAction(Request $request, $campaignUrl)
    {
        ...
        if ($request->isMethod('POST')) {
            ...
            if(!$fail){
              $user = $em->getRepository('AppBundle:User')->findOneByEmail($params['user']['email']);
              if(empty($user)){
     ...
     *
     */
      public function passwordResetAction(Request $request, $campaignUrl)
      {
          ...
          if(null !== $request->query->get('password_reset_token') && null !== $request->query->get('email')){
              ...
              $user = $em->getRepository('AppBundle:User')->findOneByEmail(urldecode($userEmail));
